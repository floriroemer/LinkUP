...
import type { AvailabilitySignal, PresenceCapabilities, PresenceRecord } from './types.js';

function normalizeCapabilities(input: unknown): PresenceCapabilities {
  const capabilities = (typeof input === 'object' && input !== null ? input : {}) as Partial<PresenceCapabilities>;
  return {
...
function createPresenceRecord(body: Record<string, unknown>): PresenceRecord {
...
function createSignal(body: Record<string, unknown>): { toPeerId: string; signal: AvailabilitySignal } {
...
async function forwardSignal(callbackUrl: string, signal: AvailabilitySignal): Promise<{ ok: boolean; status?: number; error?: string }> {
  try {
...
export function createApp(): express.Express {
  ...
  app.post('/api/presence/upsert', async (request, response) => {
    try {
      const record = createPresenceRecord(request.body as Record<string, unknown>);
  ...
  app.post('/api/notify/direct', async (request, response) => {
    try {
      const { toPeerId, signal } = createSignal(request.body as Record<string, unknown>);
      ...
      const deliveries = await Promise.all(
        routableDevices.map(async (device) => ({
